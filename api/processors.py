...
def get_filtered_events(
    ...
    if search_filter:
        filter_args = (
            Q(
                title__icontains=search_filter) | Q(
                description__icontains=search_filter) | Q(
                tags__name__icontains=search_filter) | Q(
                    organizer__icontains=search_filter) | Q(
                        location__icontains=search_filter),)

    if country_filter and country_filter not in [custom_country[
            0] for custom_country in Event.CUSTOM_COUNTRY_ENTRIES]:
...
def get_events_pending_for_report(creator):
    events = Event.objects.filter(
        creator=creator,
        reported_at=None,
        start_date__lte=datetime.datetime.now()).exclude(status='REJECTED')
